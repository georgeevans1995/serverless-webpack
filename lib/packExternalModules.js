 ...
 * @this - The active plugin instance
 */
function getProdModules(externalModules, packagePath, dependencyGraph, forceExcludes, nodeModulesPath) {
  ...
  _.forEach(externalModules, module => {
    ...
    if (moduleVersion) {
      ...
      try {
        ...
        if (fse.existsSync(modulePackagePath)) {
          if (require(modulePackagePath).peerDependencies) {
            peerDependencies = require(modulePackagePath).peerDependencies;
          }
        } else if (nodeModulesPath) {
          const rootModulePackagePath = path.join(nodeModulesPath, module.external, 'package.json');

          if (fse.existsSync(rootModulePackagePath)) {
            const rootModulePeer = require(rootModulePackagePath);

            if (rootModulePeer.peerDependencies) {
              peerDependencies = { ...rootModulePeer.peerDependencies, ...peerDependencies };
            }
          }
        }
...
module.exports = {
  ...
  packExternalModules() {
    ...
    const nodeModulesPath = this.configuration.nodeModulesPath;
    if (!includes) {
